...
public class UserBeanServiceImpl implements IUserBeanService {
	...
	public int countUserNumber(Map map) {
		...
		return userBeanDaoImpl.countUserNumber(map);
	}

	public Page getUsersListByNameAndPwd(Map map) {
		// TODO Auto-generated method stub
		
		int totalNumber=userBeanDaoImpl.countUserNumber(map);
		
		List list=userBeanDaoImpl.getUsersListByNameAndPwd(map);

		return new Page((Integer)map.get("page"), (Integer)map.get("rows"), totalNumber, list);
